...
- [Upgrading Defold versions](https://defold-typescript.github.io/toolchain/upgrading-defold-versions) — move a project between pinned surfaces, with a section per release. For 1.13.0, from 1.12.4: the removed and re-signatured Lua APIs (`model.material` removed, `liveupdate.add_mount` re-signatured, …), the source/asset migrations (Collada removal, glTF re-centering, Spine 4.6.0), and the rendering/platform default changes, each with a way to verify.
