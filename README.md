...
- **Built for the real loop** — `watch` recompiles beside the Defold editor, live transpile diagnostics surface errors inline, and source maps let you set breakpoints in your `.ts`; [`--hot-reload`](https://defold-typescript.github.io/toolchain/watch#hot-reload) pushes each successful build into the running game, whose runtime errors arrive in your terminal naming the `.ts` line that threw.
