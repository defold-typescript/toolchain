...
- [set-target](https://defold-typescript.github.io/toolchain/set-target) — write the `defold-target` pin: a fixed version, a release channel (`stable`/`beta`/`alpha`), or `--detected` to sync it to the installed Defold editor, with the typo caught at write time rather than at build time.
