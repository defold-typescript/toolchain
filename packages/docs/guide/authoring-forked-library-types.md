...
| -------- | --------------- |
| `unresolved-path` | The `moduleId`-derived path does not resolve at the pinned `ref`, including a repository that no longer exists there. No corpus target carries this verdict: `starly.starly` once did, and was lifted into measurement when its upstream turned out to have moved to a new owner rather than been deleted — so check for a relocation before recording it. |
...
say. The corpus currently carries no verdict of any kind — all 36 targets are
measured — so both reasons above are described rather than exemplified.
