...
`upgrade` is the verb that moves a project to the latest toolchain.
...
- `--json` — emit the upgrade result as a single JSON object for agents and
...
The scaffold writes this verb as a `mise` task, so once your project's `mise.toml`
is on disk the same upgrade is reachable as:
...
The task carries no version tag, so `mise` runs the `@defold-typescript/cli` that
[`init`](./init.md) pinned as a devDependency — the same version `bunx` would
resolve with `@latest`.
