...
- **`isMessage` — a helper *function*.** It is a type guard: the return type `message is BuiltinMessages[K]` is what re-introduces the lost literal at the use site.
...
- **A typo cannot compile.** `expected` is constrained to `BuiltinMessageId`, so `isMessage(message_id, message, "not_a_message")` is a compile error.
