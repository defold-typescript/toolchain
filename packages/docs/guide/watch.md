...
- **The editor may come and go.** The port is re-read on every reload, and while
  ...
  opened after the loop started is attached within about a second without a
  save. One that quits is reported once and leaves the loop rebuilding until it
  returns. Nothing more is printed while the game is not running — that is the
