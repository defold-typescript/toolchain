...
- **The editor may come and go.** The port is re-read on every reload, and while
  ...
  returns. A console connection that drops while the editor keeps running, as after the machine sleeps, is
  reopened without announcing the editor again. A reload that lands on a different editor while the old one's console
