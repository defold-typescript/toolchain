...
declare global {
  ...
  interface SceneComponentAddresses {
    "#controller": true;
    "#sprite": true;
    "/player/player#controller": true;
    "/player/player#sprite": true;
  }
}
```
...
Component ids are written in **both** forms. The same-object `#id` is offered for
every id the project declares. The object-qualified `/path#id` is offered wherever
the generator can name the object that owns the component: it reads each composed
path's own prototype — the `.go` its `instances` block names, or the payload its
`embedded_instances` block embeds — so `/player/player#sprite` is offered while
`/level#sprite` is not. A qualified key carries the same world axis a path key
does, so a component in a proxy world is offered as `mylevel:/enemy#sprite` and
never bare.

An object whose prototype the walk could not read is offered no qualified key at
all, and the reason is recorded alongside the other gaps the path walk found —
withheld rather than guessed, because an empty component set would be
indistinguishable from an object that genuinely owns none. Nothing here is a
...
An address is socket-qualified by its `:`, not by the path after it: the
grammar is `[socket:][path][#fragment]` with both halves optional, so
`mylevel:enemy`, `mylevel:` and `mylevel:#body` are checked exactly like
`mylevel:/enemy`. A `:` inside a path segment is not a socket — `/level:1/enemy`
is a bare path, because the leading `/` closes the address's head first.

The boundary is drawn on the side of silence. Nothing is reported for a
`msg.post`/`msg.url` slot, for an address that names no socket at all — a bare
`/path`, a relative `id` or `sub/id`, a lone `#fragment`, each of which
continues the caller's world by construction — for a socket that matches a world
the script runs in, or for a script whose world could not be resolved at all — a
`.ts` no scene hosts, or one whose scenes the walk could not read. A script a scene
