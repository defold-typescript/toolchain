...
- **[`build`](./build.md) and [`watch`](./watch.md) now warn when an address names a component no scene declares.** Every address literal — [`msg.post`](/api/msg)'s receiver and the rest of the classified slots — is checked against the component ids in your `.go`/`.collection` files, and an unresolvable `#fragment` is named without failing the build, on every `watch` rebuild as well as on `build`. Only the fragment is checked, never the path; a hole in the scene universe such as an unresolved [dependency](./resolve.md) reports the check as suppressed instead; and `--json` repeats each finding as a structured `unreachableAddresses` entry so an agent never parses the prose.
