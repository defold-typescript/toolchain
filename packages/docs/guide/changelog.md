...
- **A one-shot `reload` for scripts and agents.** [`reload`](./reload.md) pushes a single reload into the running game — or the editor's own extensions with `--extensions` — then reads the editor console for a bounded window (`--wait`, default 2s) and exits non-zero when the reloaded code throws, so `build && reload` reports whether the new Lua actually ran instead of only that the editor queued it. Exit 0 claims no more than that: no error was observed within the window.
