...
- **[`build`](./build.md) and [`watch`](./watch.md) fail when an emitted `require` cannot reach the output the build writes, naming the importing file, the require path, the path Defold looks for and the path the build writes.** Three shapes are caught — importing a *value* from a source with a lifecycle factory, whose output is a script component Defold never puts on the require path; any cross-file import under a configured `outDir`; and a dot in a source name, kept in the output but underscored in the require, so rename the source — while type-only imports and a `require` with no TypeScript source behind it, such as a library's Lua or `lldebugger.debug`, are left alone.
- **[`build`](./build.md) and [`watch`](./watch.md) now fail when a script's exported values cannot be told apart from its lifecycle hooks, naming the declaration and both places it is used.** Two shapes are reported — a top-level binding reassigned in one of the two and read in the other, which could not stay in sync, whatever the rebinding's spelling, and effectful top-level work with any exported declaration below it — and the fix for both is the module singleton [script state](./script-state.md#when-the-build-sends-you-here) already teaches.
