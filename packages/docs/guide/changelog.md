...
- **[nakama](/api/nakama)** — the forked surface is now measured against upstream's own Lua, and the result is worse than it looked: of the 156 functions upstream defines, only 4 are declared with the right parameter list, 16 are missing entirely, and 26 declared functions do not exist upstream at all. The numbers are committed and ratcheted so they can only improve, and [authoring forked library types](./authoring-forked-library-types.md) documents the check.
