...
- **Editor scripts get a typed `editor.*` API.** The new `@defold-typescript/types/editor-script` entrypoint carries the editor VM's `editor.get`/`command`/`transact`/`tx.*` surface and none of the game runtime, so an [editor script](./editor-scripts.md) whose `tsconfig` names it type-checks editor calls and rejects `go`/`msg`/`vmath`. The editor's own `http`/`json`/`zip` libraries and `editor.ui`/`editor.prefs` are not typed yet, so [`wall`](./wall.md) does not target editor-script directories — set `types` by hand.
