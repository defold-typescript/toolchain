...
- **[`build`](./build.md), [`watch`](./watch.md), [`run`](./run.md), [`upgrade`](./upgrade.md) and [`bob`](./bob.md) now name a newer Defold release when one exists upstream of your [pinned version](./pinning-defold-target.md).** It is checked against a per-user cache at most once a day and stays silent when you are offline; `--fail-on-drift` never escalates it, `--json` carries it as `upstreamRelease: { current, latest }`, and `--no-update-check` or a non-empty `DEFOLD_TYPESCRIPT_NO_UPDATE_CHECK` turns it off.
- **[`scene-types`](./scene-types.md) addresses now name the object and world they resolve in, cover your libraries, and stay current by themselves.**
  - **Which world an address resolves in.** A bare key like `/player` is the collection your `game.project` boots; a collection opened through a collection proxy is offered as `mylevel:/enemy`, named by that collection's own `name:`, and a collection factory's prototype is offered no static address at all, so the editor stops suggesting paths that resolve in the wrong world or nowhere. If your `game.project` cannot be read or names no `[bootstrap] main_collection`, no key is offered bare and the reason is named on the warning channel — as is a collection nothing reaches.
  - **Which object owns a component.** A component id is now offered object-qualified — `/player/player#sprite` beside the bare `#sprite`, and `mylevel:/enemy#sprite` inside a proxy world — because the generator reads each composed path's own prototype instead of pairing every object with every id, so `/level#sprite` is never suggested. An object whose prototype could not be read is offered no qualified key rather than a guessed one.
  - **Your library dependencies' addresses.** [`resolve`](./resolve.md) unpacks each dependency's `[library] include_dirs` scenes — including the `.collectionproxy` and `.collectionfactory` components that say which world a collection is, so a library collection opened through its own proxy is offered under that socket rather than reported as reached by nothing — and the component ids, object paths and animation ids in them join your project's own under the merged path Defold addresses them by, your own file winning a collision. A dependency you declared but have not resolved is now named on the warning channel of `scene-types`, [`build`](./build.md) and [`watch`](./watch.md) — and in `--json` — rather than silently skipped.
  - **Every input the declaration reads.** [`watch`](./watch.md) used to refresh the declaration on a `.go` or `.collection` save only, so retargeting a `.collectionproxy` or switching `[bootstrap] main_collection` left the wrong world's addresses on offer, while adding a `.collectionfactory` left its prototype collection still reported as reached by nothing and removing one left that warning missing for a collection nothing reached any more — until you restarted it. Those saves now regenerate too, the `game.project` one after the [`resolve`](./resolve.md) it already runs has settled, so the walk reads the dependencies that resolve just materialized.
- **The editor completes more of what you type inside a quoted name, and its [panel](./transpile-diagnostics.md) says where each suggestion is valid.**
  - **Relative addresses.** An address slot now also offers the forms valid from the file you are editing — sibling ids, deeper `sub/id` paths, and their `#fragment` joins — with the panel naming the object each one is relative to. A bare `/path` is offered only where your own script's world can reach it, so a script running behind a collection proxy is no longer suggested the bootstrap world's paths.
  - **[`model.play_anim`](/api/model)** completes its `anim_id` from the `.animationset` the model your sibling `url` addresses reads from — the ids are filenames, so an entry `/anims/idle.gltf` offers `idle`, a nested set prefixes everything it lists with its own basename, and a set reached from two different entries resolves under each rather than being refused as a cycle. A `.model` naming a mesh source directly offers nothing rather than a guess, and the panel credits the set that lists the entry producing each name, not the root set the `.model` points at.
  - **[`gui.play_flipbook`](/api/gui)** completes its `animation` from the textures the one `.gui` scene owning your gui script names — never a project-wide union — and the panel credits the atlas that really declares each one.
- **[`build`](./build.md), [`watch`](./watch.md) and your editor now flag address literals that cannot resolve.** Both checks name the file without failing the build — on `build`, on every `watch` rebuild, and on every save that moves the address universe — and `--json` repeats each finding as a structured entry, on the `sceneTypes` event too, so an agent never parses the prose. The [editor plugin](./transpile-diagnostics.md) reports the same two findings on the file you are editing as suggestions, so a broken address squiggles as you type while `tsc --noEmit` stays clean; a component universe with a hole reports nothing rather than everything, and a script no scene hosts is never reported as naming a foreign world.
  - **A component the addressed object does not have.** An address literal's `#fragment` — [`msg.post`](/api/msg)'s receiver and the rest of the classified slots — is checked against the components declared by the object its path names, so `/player#sprite` is reported even when some *other* object owns `sprite`, and the warning lists what `/player` really declares. A bare `#sprite`, a relative address, a path the scene walk cannot place, or every address alike in a project whose worlds it could not classify — a collection nothing reaches, an unreadable `.collectionproxy`, no `[bootstrap] main_collection` — is checked against your whole project's component ids instead, while a hole in the component universe such as an unresolved [dependency](./resolve.md) reports the check as suppressed; the entries are `unreachableAddresses`.
  - **A world your script cannot reach.** A socket-qualified address — `mylevel:/enemy`, and equally `mylevel:enemy`, `mylevel:` or `mylevel:#body`, since the socket is read from its `:` — written at a slot that resolves in the caller's own world, every handle-resolving [`go`](/api/go) call, is reported with the world your script actually runs in, since only `msg.post` and `msg.url` cross a collection proxy. An address naming no socket, and any script whose world the scene walk could not resolve, is never reported; the entries are `crossWorldAddresses`.

### Fixed

- **[`resolve`](./resolve.md) can no longer be made to write outside `.defold-types/dependencies/`.** A dependency archive whose scene entry traverses out with a `..` path segment could overwrite a file in your own project; such an entry is now refused and named on `stderr`, and the rest of that archive still unpacks. A directory whose name merely starts with dots — `..assets/main.collection` — is an ordinary path and unpacks normally.
