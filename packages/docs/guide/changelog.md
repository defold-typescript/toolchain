...
- **Editor scripts get a typed `editor.*` API.** The new `@defold-typescript/types/editor-script` entrypoint exports the `defineEditorScript` and `defineEditorCommand` factories and carries the editor VM's `editor.get`/`bob`/`execute`/`transact`/`tx.*` surface without the game runtime; a command wrapped in `defineEditorCommand` — or registered through `editor.command` — types its `active`/`run` opts bag from its own `query`, so a `"one"` selection reads as a single node, a `"many"` one as a list, a member the query never declared is a compile error, and both factories erase to plain command tables the editor can load. The editor's own `http`/`json`/`zip` libraries and `editor.ui`/`editor.prefs` are not typed yet, so [`wall`](./wall.md) does not target [editor-script](./editor-scripts.md) directories — set `types` by hand.
- **Editor scripts are gaining the editor VM's own libraries.** The `http`, `json`, `zip`, `zlib`, `pprint` and `tilemap.tiles` surfaces the editor VM provides are now vendored from upstream alongside `editor.*`; none of them are importable yet.
...
- **The editor completes the ids your project declares.** In an editor running the [transpile-diagnostics plugin](./transpile-diagnostics.md), the suggestions are listed after everything the editor already offers, and nothing it offers is removed.
  - **Component ids** — a caret after the `#` of an address argument offers the ids your project's `.go` and `.collection` files declare, and accepting one replaces the `#fragment` only; a caret in the path before the `#` is left to the editor alone.
