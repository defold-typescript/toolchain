...
- **An unclosed code block in the reference no longer swallows the rest of a description.** Three [resource](/api/resource) parameter and return descriptions open a Lua block upstream and never close it; the closing fence is supplied now, so an editor hover still shows the `@example` that follows and `/api/resource` renders all 24 of its symbols outside the block.
- **Ten [resource](/api/resource), [go](/api/go) and [gui](/api/gui) reference examples no longer weld a heading into the Lua.** Where the reference packs several examples into one highlight block using its own fence markers, those markers used to render as lines of code; each example is now a code block of its own with the heading as prose between them.
