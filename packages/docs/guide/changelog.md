...
- **The scaffold's managed `.gitignore`, `biome.json` and `Lua.workspace.ignoreDir` rules now follow your `tsconfig.json` `include` instead of assuming `src/`.** [`init`](./init.md) ignores generated component output by its `.ts.` suffix rather than by folder, lints the roots you actually compile, and hands the Lua language server every project-relative root your `include` names — skipping a folder that holds hand-authored `.lua`, and refusing only a root that leaves the project or is absolute. A re-init (or an [`upgrade`](./upgrade.md)) retires the old `src`-shaped rules from a project carrying them, so a generated `.lua` module shows up in `git status` unless you set `outDir`; a `biome.json` carrying comments is reported for you to edit by hand rather than rewritten, and an `upgrade` shows you each such decision whether or not the install that follows it succeeds.
