...
- **Editor scripts get a typed `editor.*` API.** The new `@defold-typescript/types/editor-script` entrypoint exports the `defineEditorScript` factory and carries the editor VM's `editor.get`/`bob`/`execute`/`transact`/`tx.*` surface without the game runtime, and an [editor script](./editor-scripts.md) command wrapped in the new `defineEditorCommand` — or registered through `editor.command` — types its `active`/`run` opts bag from its own `query`, so a `"one"` selection reads as a single node, a `"many"` one as a list, and a member the query never declared is a compile error. The editor's own `http`/`json`/`zip` libraries and `editor.ui`/`editor.prefs` are not typed yet, so [`wall`](./wall.md) does not target editor-script directories — set `types` by hand.
