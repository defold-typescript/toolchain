...
- **Forked library surfaces** — every forked library is now measured against its own upstream Lua on two separately ratcheted axes, functions and constants, or explicitly recorded as unmeasurable, so one nobody has checked can no longer read as clean: 35 of the 36 modules carry a committed parity report, and only [starly](/api/starly) says why it could not be measured. Every one of the 35 now covers upstream's whole callable surface — each function declared at the parameter count upstream defines, or recorded with upstream's own reason for leaving it out; the constants axis has gaps on three modules only, [monarch](/api/monarch.monarch), [platypus](/api/platypus) and [orthographic](/api/orthographic.camera), and a library's ambient globals, such as [deftest](/api/deftest)'s assertions and most of [boom](/api/boom), are counted apart rather than reported as names its module invented; [authoring forked library types](./authoring-forked-library-types.md) documents why a perfect score on one axis says nothing about the other.
...
- Forked libraries declare members their upstream really defines that the previous bindings left out.
  ...
  - **[orthographic](/api/orthographic.camera)** — `get_automatic_zoom` and `set_automatic_zoom`, for reading and toggling a camera's automatic zoom level.
