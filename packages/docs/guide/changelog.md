...
- **Walled directories follow your pinned Defold target.** A [pinned](./pinning-defold-target.md) surface now carries the editor-scripting API of the target that declares one, and a [wall](./wall.md) narrowed to a pinned kind resolves both its ambient namespaces and its `@defold-typescript/types/<kind>` factory import from that one surface. A target declaring no [editor-script](./editor-scripts.md) document of its own keeps the installed entrypoint, and a wall never narrows to a kind its surface did not write.
