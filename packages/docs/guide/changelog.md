...
- The `squid` library types (`paweljarosz/squid`, pinned tag `1.2`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as proto and immutable, and carry the maintained-here pin in the docs. The previous hand-written binding typed only `save_logs` and `get_config().is_enabled`; the regenerated surface keeps both — `get_config` now returns the fully typed `SquidConfig` — and adds the full logger API (`init`, `trace`/`debug`/`info`/`warn`/`error`, `log`, `new`, `set_config`) plus the `SquidInstance` handle.
