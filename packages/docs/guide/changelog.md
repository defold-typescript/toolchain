...
- **Editor scripts get a typed `editor.*` API.** The new `@defold-typescript/types/editor-script` entrypoint both exports the `defineEditorScript` factory and carries the editor VM's `editor.get`/`bob`/`execute`/`transact`/`tx.*` surface without the game runtime, so an [editor script](./editor-scripts.md) that imports the factory from it and names it in its `tsconfig` `types` type-checks the multi-argument calls upstream documents and rejects `go`/`msg`/`vmath`. The editor's own `http`/`json`/`zip` libraries and `editor.ui`/`editor.prefs` are not typed yet, so [`wall`](./wall.md) does not target editor-script directories — set `types` by hand.
...
- **[nakama.socket](/api/nakama.socket) and [nakama.session](/api/nakama.session)** complete the nakama family at the same `v3.4.0` pin: the realtime socket's chat, match, matchmaking, party and status calls, each also bound onto the socket instance `create` returns, plus the helpers that build, check, store and restore a session. [`resolve`](./resolve.md) materializes both, so the v0.24.0 note that a socket binding needed vendoring of your own no longer applies.
- **Component ids complete inside an address `#fragment`.** In an editor running the [transpile-diagnostics plugin](./transpile-diagnostics.md), putting the caret after the `#` of an address argument offers the component ids your project's `.go` and `.collection` files declare; a caret in the path before the `#` is left to the editor alone. The ids are listed after everything the editor already suggests, and accepting one replaces the `#fragment` only.
