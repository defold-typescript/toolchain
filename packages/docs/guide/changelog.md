...
- **Three defect classes are cleared from the `@example` shipped on each declaration, so many more of them compile — and build what they then use — when you copy one out of a hover or the API reference.** Examples outside these classes still carry type errors, mostly wrong property and argument types.
