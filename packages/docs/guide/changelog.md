...
- The `immutable` library types (`paweljarosz/lua-immutable`, pinned tag `v1.1`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as proto and saver, and carry the maintained-here pin in the docs. The surface follows upstream's annotations, so it differs from the previous hand-written binding: `make` now returns `unknown` rather than a generic `Readonly<T>`, so field access on its result is no longer typed, while `is_immutable` stays a boolean predicate.
