...
- [go](/api/go) and [msg](/api/msg) now document the overloads the toolchain actually ships instead of the raw reference forms: `go.get` and `go.set` lead with their curried property-key generic, `go.property` lists its eight typed forms, `msg.post` its two, and `msg.url` its three real arities rather than one all-optional signature. The committed `llms-full.txt` carries the same forms.
