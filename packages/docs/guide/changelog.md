...
- **A toolchain upgrade no longer overwrites the surface it replaces.** The materialized directory now names both axes that decide its contents — `.defold-types/defold-1.13.1@0.27.0/` — and carries the generating toolchain's stamp only once every file in it has landed, so an upgrade writes a new sibling beside the previous surface rather than replacing it, and a run cut short leaves a directory that reads as incomplete instead of one vouching for contents it does not have. See [Pinning the Defold target](./pinning-defold-target.md).
