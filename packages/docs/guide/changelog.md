...
- **[`resolve`](./resolve.md) now unpacks the scenes your library dependencies share.** Each dependency's `[library] include_dirs` collections, game objects, gui scenes and animation assets land under `.defold-types/dependencies/` at the resource path Defold addresses them by, reconciled to the live dependency set on every run and counted per dependency in `--json`. A dependency that shares nothing is named on stderr instead of passing silently.
