...
- **The editor API follows your pinned Defold target.** Each target declares its own editor-scripting surface, so a project [pinned](./pinning-defold-target.md) to a release that ships none keeps the installed [editor-script](./editor-scripts.md) entrypoint instead of silently getting the current release's editor API, and a [wall](./wall.md) narrows to a pinned surface only for the kinds that surface actually wrote.
