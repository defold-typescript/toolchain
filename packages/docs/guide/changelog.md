...
- **[`build`](./build.md) and [`watch`](./watch.md) now fail when a script's exported values cannot be told apart from its lifecycle hooks, naming the declaration and both places it is used.** Two shapes are reported — a top-level binding reassigned in one of the two and read in the other, which could not stay in sync, and effectful top-level work written above an exported declaration — and the fix for both is the module singleton [script state](./script-state.md#when-the-build-sends-you-here) already teaches.
