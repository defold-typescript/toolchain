...
- **A CLI line that reports a failure now reads `defold-typescript <command>: error: <message>`, with `error` bold red when stderr is a terminal.** A failed [`watch`](./watch.md) rebuild leads its located lines with the same kind of headline, and `--no-color`, `NO_COLOR`, `TERM=dumb` and `--json` turn color off; `--json` `error` fields are unchanged ([terminal output](./terminal-output.md)).
