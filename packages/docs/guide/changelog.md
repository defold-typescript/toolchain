...
- **The editor API follows your pinned Defold target.** A [pinned](./pinning-defold-target.md) surface now carries the editor-scripting API of the target that declares one, so an [editor-script](./editor-scripts.md) [wall](./wall.md) type-checks against that release's editor VM rather than the installed default's. A target declaring none keeps the installed entrypoint, and a wall never narrows to a kind its surface did not write.
