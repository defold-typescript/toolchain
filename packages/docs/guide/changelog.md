...
- **The scaffold now follows your `tsconfig.json` `include` and `outDir` instead of assuming `src/`.** [`init`](./init.md) seeds the starter `main.ts` under the first `include` pattern that admits it and points the generated `main/main.collection` at the component [`build`](./build.md) emits for it, reporting the patterns it searched when none reaches a writable path inside the project. The managed `.gitignore`, `biome.json` and `Lua.workspace.ignoreDir` rules read the same `include` — a re-init (or an [`upgrade`](./upgrade.md)) retires the older `src`-shaped rules, so a generated `.lua` module shows up in `git status` unless you set `outDir`, and a `biome.json` you have hand-edited to hold comments is reported for you to edit rather than rewritten.
