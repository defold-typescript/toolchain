...
- **[`build`](./build.md) and [`watch`](./watch.md) now warn about address literals that cannot resolve.** Both checks name the file without failing the build — on `build`, on every `watch` rebuild, and on every save that moves the address universe — and `--json` repeats each finding as a structured entry, on the `sceneTypes` event too, so an agent never parses the prose.
  - **A component no scene declares.** An address literal's `#fragment` — [`msg.post`](/api/msg)'s receiver and the rest of the classified slots — is checked against the component ids in your `.go`/`.collection` files, so a component you remove is reported before you touch any TypeScript. A hole in the scene universe such as an unresolved [dependency](./resolve.md) reports the check as suppressed instead; the entries are `unreachableAddresses`.
