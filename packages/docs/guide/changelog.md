...
- **[`scene-types`](./scene-types.md) addresses now name the world they resolve in, cover your libraries, and stay current by themselves.**
  ...
  - **Every save that can move an address.** [`watch`](./watch.md) used to refresh the declaration on a `.go` or `.collection` save only, so retargeting a `.collectionproxy` or `.collectionfactory` — or switching `[bootstrap] main_collection` — left the wrong world's addresses on offer until you restarted it. Those saves now regenerate too, the `game.project` one after the [`resolve`](./resolve.md) it already runs has settled, so the walk reads the dependencies that resolve just materialized.
