...
- **The main `/api/<namespace>` reference pages show the authored TypeScript examples instead of Lua.** 329 functions carry a hand-authored body that previously rendered only on the version-pinned pages such as `/api/defold-1.13.1/resource`; a function with no authored body for its example still shows the reference's own Lua — see [resource](/api/resource).
