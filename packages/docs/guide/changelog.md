...
- **Defold 1.13.0 is a shipped [API target](./pinning-defold-target.md) again** — `set-target 1.13.0` resolves, `build` materializes its complete pre-baked surface including its own [editor-script](./editor-scripts.md) API, and the `/api/defold-1.13.0/…` reference pages are back, so a project rolling the engine back gets 1.13.0's editor surface rather than the installed release's. Version notes across the [API reference](/api) and search now name the release that actually introduced or changed a symbol — [`compute`](/api/compute) reads `Since Defold 1.13.0` rather than `Since Defold 1.13.1`.
- **A toolchain upgrade no longer overwrites the surfaces it replaces.** Both materialized directories now name the toolchain that wrote them — `.defold-types/defold-1.13.1@0.27.0/` and `.defold-types/libraries@0.27.0/` — and carry its stamp only once every file in them has landed, so an upgrade writes new siblings beside the previous surfaces rather than replacing them, your tsconfig moves to the new names, and a run cut short leaves a directory that reads as incomplete instead of one vouching for contents it does not have. A repeat [`resolve`](./resolve.md) whose dependency set and toolchain are both unchanged now reuses the library surface instead of rewriting it. See [Pinning the Defold target](./pinning-defold-target.md).
...
- **A [Defold pin](./pinning-defold-target.md) now holds, and says so when it cannot.** Three ways it used to fail quietly:
  ...
  - **Package imports** — the pin repointed only the ambient type roots, so a single idiomatic `import { defineScript } from "@defold-typescript/types"` anywhere in the project merged the *current* API surface back over the pinned one for every file, and a project pinned to 1.12.4 still compiled [`collectionproxy.load`](/api/collectionproxy). A pin now binds the package specifier itself — an API the pinned surface lacks is a compile error, and the bound specifier serves the package's complete export set, so pinning no longer strips symbols such as `parseDefoldApiDoc` from `@defold-typescript/types`.
...
- **Code examples on an older [Defold surface](./pinning-defold-target.md) render TypeScript again.** A project pinned to 1.13.0 or 1.12.4 saw raw Lua in the `@example` block of [`gui.set`](/api/gui) and six other calls; every committed surface is now checked, so a Defold bump cannot silently regress one.
- **A matched vendored library's types survive the next build.** Once the library surface started naming its toolchain version, the [`build`](./build.md) that followed a [`resolve`](./resolve.md) dropped the `libraries@<version>` entry from your `tsconfig.json`, so every library symbol stopped resolving until you ran `resolve` again.
