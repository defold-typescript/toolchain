...
- **[`scene-types`](./scene-types.md) addresses now name the world they resolve in, cover your libraries, and stay current by themselves.**
  ...
  - **Every input the declaration reads.** [`watch`](./watch.md) used to refresh the declaration on a `.go` or `.collection` save only, so retargeting a `.collectionproxy` or switching `[bootstrap] main_collection` left the wrong world's addresses on offer, and adding or removing a `.collectionfactory` reference left its prototype collection reported as reached by nothing, until you restarted it. Those saves now regenerate too, the `game.project` one after the [`resolve`](./resolve.md) it already runs has settled, so the walk reads the dependencies that resolve just materialized.
