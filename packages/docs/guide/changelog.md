...
- **The API reference shows authored TypeScript examples.** 329 functions carry a hand-authored body, and both the canonical `/api/<namespace>` pages and the version-pinned ones such as `/api/defold-1.13.1/resource` render it wherever the example's source hash resolves; an example with no resolving authored body shows the reference's own Lua — see [resource](/api/resource).
- **Every documented constant enum has a name you can write.** A parameter that spelled out 41 branded `gui.EASING_*` arms reads `gui.Easing` — exported from the namespace it belongs to ([gui](/api/gui), [go](/api/go), [graphics](/api/graphics), [render](/api/render), [window](/api/window) and the rest), with separate names where a getter and its setter accept different members, such as [window](/api/window)'s `DimModeState` and `DimModeStateSettable` — so signatures and hovers stay legible and your own wrapper can take one instead of falling back to `number`. Each alias has an API-reference entry listing its members, and every mention links to it from the signature and from the Parameters and Returns breakdown beneath it, landing on the entry for the version you are reading.

### Fixed

- **Four defect classes are cleared from the `@example` shipped on each declaration, so many more of them compile — and build what they then use — when you copy one out of a hover or the API reference.** Examples outside these classes still carry type errors, mostly wrong property and argument types.
  ...
  - **Undeclared names** — 49 functions used names the example itself never introduced, leaving `srcbuffer`, `url` or `self` undefined. Each body declares every ordinary local with the real API that produces it, gets `self` the way real code does — from the script factory its own kind exports, or as a parameter where the example is documented across several kinds — and what it declares is a resource the rest of the body validly uses: a [render](/api/render) target is freed once from a teardown hook and declares every attachment it is queried for, and a [resource](/api/resource) array texture narrows the resource and image it loads before reading them, holds every page written to it, and is created at the dimensions of the image it decodes — on every pinned [`defold-target`](./pinning-defold-target.md) that ships the example.
  ...
  - **Several examples run together** — 35 functions, across [go](/api/go), [gui](/api/gui), [render](/api/render), [resource](/api/resource) and the script callbacks, document more than one example and shipped them concatenated into a single block; 15 of those blocks stacked two or more `export default` and could never compile, and the other 20 compiled only as one welded program, with the sentences separating the examples folded into the fence as comments and colliding names renamed to keep it valid. Each example is now its own `@example` block in the hover and on the API reference, led by the sentence the reference writes above it, and carrying everything that example needs on its own.
- **Reference descriptions reach you whole, in an editor hover and on the API reference.** Three ways the reference's own markup broke the render are repaired in the one converter both surfaces read.
  - **Unclosed code block** — three [resource](/api/resource) parameter and return descriptions open a Lua block upstream and never close it, swallowing everything after it; the closing fence is supplied now, so a hover still shows the `@example` that follows and `/api/resource` renders all 24 of its symbols outside the block.
  - **Heading welded into the Lua** — ten [resource](/api/resource), [go](/api/go) and [gui](/api/gui) examples pack several examples into one highlight block using the reference's own fence markers, which rendered as lines of code; each example is a code block of its own now, with the heading as prose between them.
  - **Bullet with no value** — 41 [resource](/api/resource) bullets showed an empty `-` with the value they described adrift below the list, on every pinned [`defold-target`](./pinning-defold-target.md); each marker now carries its value.
...
- **A pure-Lua library that ships its own `.script_api` now gets this toolchain's curated types instead of an ambient namespace generated from that doc.** [`resolve`](./resolve.md) matches each dependency against the vendored corpus first, so a confirmed match such as [boom](/api/boom) or [bridge](/api/bridge) is imported by its `require` path (`import * as boom from 'boom.boom'`) and the stale `extensions/` namespace an earlier run wrote is pruned with its tsconfig entry. A match the archive does not confirm changes nothing: it is reported as an unverified library only when the dependency is asset-only, and otherwise keeps its own `.script_api` namespace with no entry and no warning.
- **[`init`](./init.md) writes no debug file; [`setup-debug`](./debugging.md) writes the whole debug path.** The `launch.json` configuration, the `defold-debug.ts` Bun launcher and the Local Lua Debugger recommendation arrive with the `game.project` dependency, the ambient declaration and the entry-script bootstrap, so a scaffolded project stops advertising a debugger nothing has wired; a `launch.json` the command cannot parse stops it by name with the project left untouched. [`upgrade`](./upgrade.md) and [`init . --force`](./init.md) refresh the two derived fields of a launch configuration already present and create none.
...
- **The scaffold now follows your `tsconfig.json` `include` and `outDir` instead of assuming `src/`.** [`init`](./init.md) seeds the starter `main.ts` under the first `include` pattern that admits its own `main.ts` — an exact-file `include` such as `["src/main.ts"]` among them — and points the generated `main/main.collection` at the component [`build`](./build.md) emits for it, reporting the patterns it searched instead of scaffolding when none of them does. The managed `.gitignore`, `biome.json` and `Lua.workspace.ignoreDir` rules read the same `include` — a re-init (or an [`upgrade`](./upgrade.md)) retires the older `src`-shaped rules, so a generated `.lua` module shows up in `git status` unless you set `outDir`, and a `biome.json` you have hand-edited to hold comments is reported for you to edit rather than rewritten.
...
- **[`upgrade`](./upgrade.md) and [`init . --force`](./init.md) now leave a current `.vscode/launch.json` exactly as you wrote it.** Both rewrote the file from its parse tree on every run, dropping your `//` comments and hand indentation even when neither derived field had changed.
...
- **[Debugging](./debugging.md) now follows your configured sources instead of assuming `src/`, on both ends.** [`setup-debug`](./debugging.md) picks the entry script, cleans up stale blocks and writes the ambient `lldebugger.debug.d.ts` next to that script off your `include`, refusing a target that is not a configured production source by name and cause — outside the patterns, a declaration file, or under a generated or dependency tree such as `build`/`node_modules`; and the `launch.json` it writes derives its `scriptFiles` and `scriptRoots` from `include` and `outDir`, so a breakpoint binds in a gui script, a render script or a plain `.lua` module wherever the build writes it. [`upgrade`](./upgrade.md), and [`init . --force`](./init.md), rewrite those two fields in place when your inputs move, leaving the rest of the configuration alone.

## v0.36.0

### Breaking

- **[`render.clear`](/api/render)'s buffer table is now keyed by the three `graphics.BUFFER_TYPE_*` constants its reference names, not by any number.** `LuaMap<number, number | Vector4>` no longer compiles — `LuaMap`'s key type is invariant — so declare it `LuaMap<render.ClearBufferKey, number | Vector4>`, an alias exported on the default surface and every pinned `defold-target` ([Lua table constructs](./lua-table-constructs.md)).
- **[`build`](./build.md) and [`watch`](./watch.md) now reject three output arrangements they used to emit silently, each naming the file and what to change.** `build` exits non-zero having written no Lua; `watch` reports the same diagnostics and keeps watching, rebuilding once you fix the file. Type-only imports, and a `require` with no TypeScript source behind it such as a library's Lua or `lldebugger.debug`, are still left alone.
  - **Unreachable `require`** — every emitted `require` is checked against the outputs the build writes, naming the importing file, the path Defold looks for and the path written. A dot in a source name is rejected rather than repaired, so rename the source.
  - **Unsplittable script** — a script whose exported values cannot be told apart from its lifecycle hooks: a top-level binding reassigned in one and read in the other, whatever the rebinding's spelling, or effectful top-level work above an exported declaration. Both are fixed by the module singleton [script state](./script-state.md#when-the-build-sends-you-here) already teaches.
  - **Contested output path** — a path claimed by two sources, or one already holding Lua the build did not generate, reported before anything is written so that file is left byte-for-byte alone. The generated `lualib_bundle.lua` and `defold_typescript_timers.lua` are inside the same contract, so a hand-authored file at either output-root path has to be renamed.

### Improved

- **A script can now export runtime values other sources import.** A source that calls a lifecycle factory *and* exports values builds into two files — the component, and a companion Lua module beside it (`src/door.ts` -> `src/door.ts.script` and `src/door.lua`) carrying exactly the exports and the declarations they reach, so a value only the script uses stays in the script alone. The component requires the companion rather than redeclaring it, so an exported object is one table for the script and every importer, under a configured `outDir` as much as alongside the sources ([script state](./script-state.md#exporting-a-value-from-a-script-itself)).
- **[Lua table constructs](./lua-table-constructs.md) is a new guide page on writing a table the engine reads by constant key**, such as [`render.clear`](/api/render)'s buffer table: why the object literal does not compile, the checked `LuaMap` route and the inline cast, and the mapped type that looks like the fix while silently accepting a key from another family.
- **[Script lifecycle](./script-lifecycle.md#three-ways-to-type-self) now lays out all three ways to type `self` and what inference costs.** It covers `init(self): State` and `defineScript<Props, State>` beside the inferred default, why one type argument cannot sit next to `properties`, and that a type argument describes state without creating it — a field no `init` writes is `nil` on a fresh instance.

### Fixed

- **A project with a configured `outDir` now loads what [`build`](./build.md) writes.** Every emitted `require` — a cross-file import, and the generated `lualib_bundle.lua` and `defold_typescript_timers.lua` — used to keep its source-tree spelling while the file was written under the `outDir`, so Defold found nothing at load time; each now names the path the build wrote. An `outDir` whose own name contains a dot can be named by no `require` at all and fails the build before anything is written.
- **A timers-only project now gets the `lualib_bundle.lua` its runtime requires.** The generated timer runtime has always required the bundle, but nothing asked for it to be written unless some source also used a helper like `Object.keys`, so such a project loaded a file the build never wrote.
- **The [`font`](/api/font) namespace now exists on the default surface.** It shipped in the per-kind subpaths and every pinned `defold-target` but was missing from the package's own entrypoint, so `font.add_font(...)` failed to compile for a project that imports `@defold-typescript/types` directly.
- **Three API reference code examples now compile.** [`render.clear`](/api/render) passed an object literal where the signature takes a `LuaMap`, [`zlib.inflate`](/api/zlib)'s sample payload used string escapes TypeScript rejects outright, and [`factory.create`](/api/factory) put two `export default`s in one block; each is now copy-pasteable, and every hand-authored example is type-checked against the surfaces that ship it so a new one cannot regress.
...
- **`@defold-typescript/types` no longer declares [`iac`](/api/iac), [`iap`](/api/iap), [`push`](/api/push) or [`webview`](/api/webview), on the default surface or any pinned `defold-target`.** A project that calls them must add the extension as a `dependencies#N` entry in `game.project` and run [`resolve`](./resolve.md), which types them like every other extension ([native extensions](./extensions.md)).
...
- **Hand-authored engine overloads now show every overload form on the canonical API reference pages.** [`go.get`](/api/go), [`go.set`](/api/go), [`go.property`](/api/go), [`msg.post`](/api/msg), [`msg.url`](/api/msg), [`render.render_target`](/api/render) and the [`vmath`](/api/vmath) generics list each arm on their unversioned `/api/<namespace>` page, which previously showed a single stale generated signature; the same arms are one entry each in `llms-full.txt`.
...
- **Every Defold library is now listed under [Libraries](/libraries) › defold, which leads the sidebar and the Libraries page marked `(official)`.** Each `.script_api` a `defold` GitHub org repo ships gets a page at its pin, with setup steps matching that release, tag or commit, and each repo with no typed API gets its own icon-marked page and sidebar entry, such as [extension-qrcode](/libraries/defold/extension-qrcode), describing what it ships with the same GitHub pin and setup steps as a typed library page — except that placeholder repos show no install step and fork-only repos start with forking; [`iac`](/api/iac), [`iap`](/api/iap), [`push`](/api/push) and [`webview`](/api/webview) moved there from the engine reference, and their `/api/` links still work.
- **Defold library pages show each extension's constants, code examples, callback arguments and optional parameters.** Pages such as [`admob`](/api/admob) and [`iap`](/api/iap) carry a Constants section and the upstream usage examples, list a callback's arguments and a table's fields nested under their parameter, and mark optional parameters with `?`.
- **Lua and TypeScript code samples in the guide and API reference are labelled with their language.** A small badge shows the language's logo and name beside a sample's file name, or on the block's top edge when it has none, and copying a sample never includes it.
- **Platform and caution markers in the API reference now show as platform logos and icons whose label appears on hover or keyboard focus, with caution icons tinted like a warning callout.** Notes such as "iOS only" or "attention" on pages like [`sys`](/api/sys) and [`iap`](/api/iap) used to vanish or read as `[icon:ios]` bracket text; library cards and symbol tooltips now drop the brackets.
...
- **[`webview.create`](/api/webview), `webview.eval` and `webview.is_visible` now return `number` instead of `void`.** The webview id, request id and visibility flag they hand back can be stored and passed on without a cast.
...
- **Refreshing the bundled library pins changes declarations a project may already be calling.**
  - **[druid](/api/druid)** breaks two call sites at `1.3.1`: text `set_text` now requires the new text as an argument and returns the component, so a bare `text:set_text()` no longer compiles, and a button's `style` is now the typed `druid_button_style` rather than a plain `LuaTable`, so a value declared `LuaTable` no longer assigns to it.
  - **[decore](/api/decore) changes six declarations across `4` -> `6`, two of them without a compiler error.**
    - **`event_bus.process_all` and the tiny-ecs `system.nocache` opt-out are gone.** `process_all` folded into `process`, and `nocache` went with the shape cache that replaced it; both were call-site-visible, so a project using either must move before taking this release.
    - **`event_bus.trigger` now takes the entity second.** A written `trigger(name, payload)` binds the payload to the entity slot — move it to the third argument. The compiler often will not catch this, because every field of `entity` is optional.
    - **`event_bus.process` calls back once per entity.** The callback takes `(entity, data)` rather than the event list, and the call returns both the events and their entities.
    - **`event_bus.set_merge_policy` and `merge_callbacks` re-shaped the merge callback** to `(entity, data, datas, entity_map)`. An existing inline callback keeps compiling with every parameter silently re-bound, so read each body rather than trusting a green build.
    - **`event_bus.get_events` and `get_stash` changed shape.** `get_events` now requires the event name its old declaration wrongly omitted, and `get_stash` yields `unknown` elements rather than `LuaTable`, so using one as a table needs a cast.
    - **`world.findEntities` requires the `out` buffer.** A three-argument call through the `world` field needs an explicit `undefined` fourth argument; the module-level `find_entities` takes the same buffer optionally.
  ...
  - **[log](/api/log) no longer declares `get_default_logger_name`.** Upstream moved it out of the public module at `7`, and there is no replacement — the default logger name is now derived internally — so a project calling it must supply its own name to `get_logger`.

### Improved

- **The bundled library pins are being refreshed against their current upstream releases.**
  - **[druid](/api/druid)** moves from `1.2.5` to `1.3.1`, adding `get_value`/`set_value` on progress, `is_enabled`/`set_enabled` on swipe, `set_value` on timer, `set_justify`/`is_justify` on rich text, `add_drag_action`/`remove_drag_action` on drag, `refresh` on data list and `get_text_visual` on input. Several existing declarations widened without breaking a call site — `get_widget` takes an optional `template`, the druid instance's `set_whitelist`/`set_blacklist` made their argument optional, and a button `callback` now receives the caller, the custom args and the button — while the internal `_on_mouse_hover` on scroll is gone.
  ...
  - **[log](/api/log)** moves from `6` to `7`, adding a file-output API (`set_file`, `get_file`, `clear_log_files`, `final` and the per-logger `set_file_nearby`) and custom log handlers (`add_callback`, `remove_callback`, `clear_callbacks`). The level methods now accept a nil message, and `get_default_logger_name` is gone.
  - **[panthera](/api/panthera)** moves from `runtime.8` to `runtime.10`, a purely additive release: the animation state gains the fields the new clip playback path exposes (`clips`, `template_states`, `play_animation`, `play_options`, `play_sample_depth` and a reusable `collect_buffer`), and three supporting types are now declared. Nothing was removed and no existing declaration changed, so no call site moves.
  - **[bridge](/api/bridge)** moves from `v2.0.0` to `v2.1.0`, adding a `notifications` API: `schedule` a notification the platform shows after the player leaves the game, `cancel` one by id or `cancel_all`, and `is_supported` to check the platform first. Nothing was removed and no existing declaration changed.
...
- **Your project's own message ids can now carry a typed payload.** Declare them as string keys of the ambient `CustomMessages` interface and [`msg.post`](/api/msg), `isMessage` and `onMessage` check and narrow them exactly like a built-in message, while the emitted Lua stays byte-for-byte what it was. [messages](./messages.md) carries the recipe, a worked cross-object example, and where a declaration may live — beside the payload type it names, or in a mapping-table `.d.ts` — along with the two shapes that quietly do not register.

### Fixed

- **The [`camera`](/api/camera) and [`sprite`](/api/sprite) property catalogs now report the types the engine really uses.** All eight `camera.properties` members read as `unknown`, so every `go.get<camera.properties>()` needed a cast; they are now `number`, with `camera.projection` and `camera.view` typed `Matrix4` and `sprite.frame_count` typed `number` where the reference data says otherwise.

## v0.31.0

### Improved

- **[`build`](./build.md), [`watch`](./watch.md), [`run`](./run.md), [`upgrade`](./upgrade.md) and [`bob`](./bob.md) name a newer Defold release when one exists upstream of your [pin](./pinning-defold-target.md).** Checked at most once a day against a per-user cache, advisory only, and silent when you are offline; `--json` carries it as `upstreamRelease: { current, latest }`, and `--no-update-check` or `DEFOLD_TYPESCRIPT_NO_UPDATE_CHECK` turns it off.
- **[`scene-types`](./scene-types.md) addresses now name the object and world they resolve in, cover your libraries, and stay current by themselves.**
  - **Worlds.** A collection opened through a collection proxy is offered as `mylevel:/enemy` rather than a bare path, and a collection factory's prototype is offered no static address, so the editor stops suggesting paths that resolve in the wrong world.
  - **Objects.** Component ids are offered object-qualified — `/player/player#sprite` beside the bare `#sprite` — so an id is never suggested on an object that does not own it.
  - **Libraries.** [`resolve`](./resolve.md) unpacks each dependency's scenes and merges their component ids, object paths and animation ids into your own under the address Defold uses, your file winning a collision. A dependency you declared but have not resolved is now named on the warning channel rather than silently skipped.
  - **Freshness.** Saving a `.collectionproxy`, `.collectionfactory` or `game.project` regenerates the declaration too, so `watch` no longer serves the wrong world's addresses until you restart it.
- **The editor completes more of what you type inside a quoted name, and its [panel](./transpile-diagnostics.md) says where each suggestion is valid.**
  - **Relative addresses.** An address slot also offers the forms valid from the file you are editing — sibling ids, deeper `sub/id` paths and their `#fragment` joins — while a bare `/path` is offered only where your own script's world can reach it.
  - **[`model.play_anim`](/api/model).** Its `anim_id` completes from the `.animationset` the model your sibling `url` addresses reads from, credited to the set that really lists each entry.
  - **[`gui.play_flipbook`](/api/gui).** Its `animation` completes from the textures of the one `.gui` scene owning your gui script, never a project-wide union.
- **[`build`](./build.md), [`watch`](./watch.md) and your editor flag address literals that cannot resolve, naming the file without failing the build.** A `#fragment` is checked against the components the addressed object really declares (`unreachableAddresses`), and a socket-qualified address such as `mylevel:/enemy` written at a slot that resolves in the caller's own world is reported with the world your script actually runs in (`crossWorldAddresses`). `--json` repeats both as structured entries, and the [editor plugin](./transpile-diagnostics.md) shows them as suggestions so a broken address squiggles as you type while `tsc --noEmit` stays clean.

### Fixed

- **[`resolve`](./resolve.md) can no longer be made to write outside `.defold-types/dependencies/`.** A dependency archive entry that traverses out with a `..` segment is refused and named on `stderr`, and the rest of that archive still unpacks; a directory merely starting with dots is an ordinary path.
- **[`run`](./run.md) no longer launches a cached stock engine and a build that record different Defold versions** — the pairing that made Defold report `FORMAT_ERROR` after a [pin](./pinning-defold-target.md) bump. It now reports the two versions instead of launching; an engine cached before this release keeps launching until one [`bob run`](./bob.md) replaces it.

## v0.30.0

### Improved

- **[`scene-types`](./scene-types.md) turns your project's scenes into completions, and keeps them current on its own.** The game-object paths and component ids in every `.collection` and `.go` land in `.defold-types/scene-addresses.d.ts`, which [`init`](./init.md) and [`upgrade`](./upgrade.md) name in your `tsconfig.json`, [`build`](./build.md) refreshes before it transpiles, and [`watch`](./watch.md) refreshes on every scene save — so address slots like `msg.post`'s receiver suggest real addresses while still accepting one composed at runtime, and you run the verb yourself only for a one-off or a CI step that runs neither.
...
- **A typedef in the [API reference](/api) now has its own heading, and signatures link to it.** Each shape a page documents gets a heading of its own in the "On this page" list instead of sharing one `Types` section, and a parameter or return type naming one of that page's shapes links straight to that shape's heading — including when the shape's name matches an earlier heading on the same page.
- **The [API reference](/api) version range selector reads at a glance.** The closed `From` / `To` chrome now shows the bare version behind a direction arrow instead of the word label plus `Defold 1.13.0`, and the full `Defold <semver>` label still names every option once the dropdown is open.

### Fixed

- **A [pinned Defold target](./pinning-defold-target.md) now gets the same typed surface as an unpinned project.** Every surface the toolchain writes for a pin was missing the hand-authored augmentations — the [`vmath`](/api/vmath), [`go`](/api/go) and [`msg`](/api/msg) overloads, the message and window-event guards, and the global `hash`/`pprint` declarations — along with the built-in message table those guards and overloads are written against, so code that compiled unpinned stopped compiling once you pinned and `msg.post` payload checking quietly fell back to `any`.
...
- **The [API reference](/api) can be read over a range of Defold releases, not just one.** Paired `From`/`To` dropdowns pick the range: every page and index then covers it whole, narrowing `From` drops the symbols that had already gone by then, and moving either bound keeps you on the page you were reading whenever the new range still covers it. See [Reading the API reference over a range](./upgrading-defold-versions.md#reading-the-api-reference-over-a-range).
  ...
  - **Badges** — the `New`/`Changed`/`Deprecated` markers and every count beside them, sidebar included, now describe movement inside the range you picked rather than across all tracked releases, so a same-release range marks nothing `New` or `Changed` while `Deprecated` still follows the `To` bound and can mark it. Availability notes stay absolute, and the sidebar counts carry every category the range can reveal and announce the tally they are currently showing.

## v0.27.0

### Improved

- **Nothing you were already using disappears when something upgrades.** A Defold patch release and a toolchain upgrade both used to overwrite the surface they replaced; each now lands beside it. See [Pinning the Defold target](./pinning-defold-target.md).
  - **Defold targets** — the target a patch replaces is demoted to a committed historical surface rather than overwritten, so it keeps resolving through `set-target`. The toolchain ships the current release plus the previous minor pre-baked, and a bump reports every version that leaves that set.
  - **Defold 1.13.0** — a shipped API target again: `set-target 1.13.0` resolves, and `build` materializes its complete pre-baked surface, including its own [editor-script](./editor-scripts.md) API and the `/api/defold-1.13.0/…` reference pages.
  - **Materialized directories** — `.defold-types/defold-1.13.1@0.27.0/` and `.defold-types/libraries@0.27.0/` now name the toolchain that wrote them, and are stamped only once every file has landed. Your tsconfig moves to the new names, and a run cut short reads as incomplete instead of vouching for contents it does not have.
- **Version notes name the release that actually changed a symbol.** Across the [API reference](/api) and its search, [`compute`](/api/compute) reads `Since Defold 1.13.0` rather than `Since Defold 1.13.1`.
- **A repeat [`resolve`](./resolve.md) reuses the library surface** when your dependency set and the toolchain are both unchanged, instead of rewriting it.

### Fixed

- **A [Defold pin](./pinning-defold-target.md) now holds, and says so when it cannot.** Five ways it used to fail quietly:
  - **`set-target`** — a version that never existed, such as `1.42.99`, printed a confident transition line and exited 0. The pin is now written only when the API registry can provide the version, and otherwise rejected with the resolvable targets listed, writing nothing either way; `--detected` is checked identically, and channels still pass through.
  - **`build`, `run`, `watch`, `upgrade`, `bob`** — a pin with no shipped API surface materialized nothing and exited 0 while your code kept compiling against the newer default. They now name the pin and the resolvable targets on stderr and in the `--json` `warnings`, and `--fail-on-drift` turns it into a non-zero exit.
  - **Package imports** — one idiomatic `import { defineScript } from "@defold-typescript/types"` merged the *current* API surface back over the pinned one for every file, so a project pinned to 1.12.4 still compiled [`collectionproxy.load`](/api/collectionproxy). A pin now binds the package specifier itself, and still serves the package's complete export set.
...
- **The per-version upgrade page is now one evergreen guide.** [Upgrading Defold versions](./upgrading-defold-versions.md) replaces `upgrading-to-defold-1-13-0.md` with a version-agnostic runbook plus one `## Defold <version>` section per release, each keeping that release's own "what changed" notes, so the old page's URL no longer resolves. Each release's per-symbol notes are individually linkable headings and appear in the page's table of contents.
