...
- The upstream-migration fidelity gate is stricter about when a documentation source may replace a library's existing types, so no migration can quietly downgrade a surface you already depend on.
  - **No parameter or return types** — a source that names a library's functions but documents none of their signatures previously scored as a clean match and would have swapped a fully-typed surface for argument-less stubs. [persist](/api/persist.persist) was the first library to hit this and keeps its existing types.
