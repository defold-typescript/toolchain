...
- **Every documented constant enum has a name you can write.** A parameter that spelled out 41 branded `gui.EASING_*` arms now reads `gui.Easing`, so signatures, hovers and the API reference stay legible, and because each alias is exported from the namespace it belongs to — [gui](/api/gui), [go](/api/go), [graphics](/api/graphics), [render](/api/render), [window](/api/window) and the rest — your own wrapper can take one as a parameter instead of falling back to `number`. Where a getter and its setter accept different members they get separate names, such as [window](/api/window)'s `DimModeState` and `DimModeStateSettable`, and every alias has an API-reference entry listing its members.
