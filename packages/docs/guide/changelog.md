...
- **[`build`](./build.md) and [`watch`](./watch.md) now warn when an address names a component no scene declares.** Every address literal's `#fragment` — [`msg.post`](/api/msg)'s receiver and the rest of the classified slots — is checked against the component ids in your `.go`/`.collection` files and named without failing the build: on `build`, on every `watch` rebuild, and and on every save that moves the address universe, so a component you remove is reported before you touch any TypeScript. A hole in the scene universe such as an unresolved [dependency](./resolve.md) reports the check as suppressed instead, and `--json` repeats each finding as a structured `unreachableAddresses` entry — on the `sceneTypes` event too — so an agent never parses the prose.
