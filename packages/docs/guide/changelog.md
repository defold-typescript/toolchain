...
- The explicit type-argument factory form the [script lifecycle](./script-lifecycle.md) guide documents — `export default defineGuiScript<MenuSelf>({ … })` — now builds to its component file (`src/menu.ts.gui_script`), where it previously fell through to a plain `.lua` module of the wrong kind. A type-only name imported alongside the factory (`import { defineGuiScript, type Hash }`) also no longer fails the build with an unresolvable `require` of `@defold-typescript/types`.
