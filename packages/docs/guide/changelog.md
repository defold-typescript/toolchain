...
- **[`build`](./build.md) and [`watch`](./watch.md) now reject three output arrangements they used to emit silently, each naming the file and what to change.** `build` exits non-zero having written no Lua; `watch` reports the same diagnostics and keeps watching, rebuilding once you fix the file. Type-only imports, and a `require` with no TypeScript source behind it such as a library's Lua or `lldebugger.debug`, are still left alone.
  - **Unreachable `require`** — every emitted `require` is checked against the outputs the build writes, reporting the importing file, the require path, the path Defold looks for and the path written. One shape is rejected rather than repaired: a dot in a source name, kept in the output but underscored in the require, so rename the source.
  - **Unsplittable script** — a script whose exported values cannot be told apart from its lifecycle hooks, naming the declaration and both places it is used: a top-level binding reassigned in one of the two and read in the other, whatever the rebinding's spelling, and effectful top-level work with any exported declaration below it. The fix for both is the module singleton [script state](./script-state.md#when-the-build-sends-you-here) already teaches.
  - **Contested output path** — a path claimed by two sources, or one already holding Lua the build did not generate, reported before anything is written so the contested file is left byte-for-byte alone. A `watch` rebuild judges this against the whole project rather than only the sources it recompiled, and the generated `lualib_bundle.lua` and `defold_typescript_timers.lua` are inside the same contract, so a hand-authored file at either output-root path — a bundle left by another toolchain, say — has to be renamed.

### Improved

- **A script can now export runtime values other sources import.** A source that calls a lifecycle factory *and* exports values builds into two files — the component, and a companion Lua module beside it (`src/door.ts` -> `src/door.ts.script` and `src/door.lua`) carrying exactly the exports and the declarations they reach, so a value only the script uses, and any effect its initializer runs, stays in the script alone. The component requires the companion rather than redeclaring it, so an exported object is one table for the script and every importer, under a configured `outDir` as much as alongside the sources ([script state](./script-state.md#exporting-a-value-from-a-script-itself)).
...
- **A project whose only use of a runtime helper is the timers module now gets the `lualib_bundle.lua` that module needs.** The generated timer runtime has always required the bundle, but nothing asked for it to be written unless some source also used a helper like `Object.keys`, so a timers-only project loaded a file [`build`](./build.md) never wrote; importing the timers module now writes and relocates the bundle like any other helper use, on both `build` and [`watch`](./watch.md).
