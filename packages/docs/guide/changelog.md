...
- The guide no longer suggests that a sufficiently pure npm package can work: [TypeScript vs Lua](./typescript-vs-lua.md)'s "self-contained Lua" claim is corrected, and [TypeScript gotchas](./typescript-gotchas.md) gains an entry showing the exact build failure, why `node_modules` is never read, and how to vendor the source under `src/` instead. That entry now shows the output paths a scaffolded project actually produces — `src/vendor/pure-pkg.lua` beside `src/main.ts.script` — and states that include-base stripping runs only when `outDir` names a directory other than `.`, since an absent, empty, or `.` `outDir` all emit alongside the source.
...
- The upstream-migration fidelity gate no longer approves a documentation source that names a library's functions but documents none of their parameters or return types — it previously scored such a source as a clean match and would have replaced a fully-typed surface with argument-less stubs. [persist](/api/persist.persist) was the first library to hit this and keeps its existing types.
- That same guide's [defsave](/api/defsave) worked example had the retired binding's member counts wrong (7 functions plus `appname`, not 8 functions and no fields), and its "100% fidelity" claim now reads accurately: the identity diff proves the emit is lossless, not that the surface matches upstream.
