...
- Forked libraries declare members their upstream really defines that the previous bindings left out.
  ...
  - **[defcon](/api/defcon)** — `register_module` takes upstream's optional second `name` argument.
  ...
  - **[nakama](/api/nakama)** — the subscriptions area (`list_subscriptions`, `get_subscription`, the `validate_subscription_*` calls and their constructors), the cancellation pair `cancellation_token` and `cancel`, plus `delete_account` and `delete_tournament_record` — 16 members upstream defines that the previous binding never declared.
