...
- **A toolchain upgrade no longer overwrites the surface it replaces.** The materialized directory now names both axes that decide its contents — `.defold-types/defold-1.13.1@0.27.0/` — and stamps the generating toolchain into its `package.json`, so upgrading the toolchain writes a new sibling and leaves the previous surface on disk to diff against. See [Pinning the Defold target](./pinning-defold-target.md).
