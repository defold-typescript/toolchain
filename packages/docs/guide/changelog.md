...
- **[`build`](./build.md) and [`watch`](./watch.md) now reject three output arrangements they used to emit silently, each naming the file and what to change.** `build` exits non-zero having written no Lua; `watch` reports the same diagnostics and keeps watching, rebuilding once you fix the file. Type-only imports, and a `require` with no TypeScript source behind it such as a library's Lua or `lldebugger.debug`, are still left alone.
  - **Unreachable `require`** — every emitted `require` is checked against the outputs the build writes, reporting the importing file, the require path, the path Defold looks for and the path written. Two shapes remain reachable: any cross-file import under a configured `outDir`, and a dot in a source name, kept in the output but underscored in the require, so rename the source.
...
- **A script can now export runtime values other sources import.** A source that calls a lifecycle factory *and* exports values builds into two files — the component, and a companion Lua module beside it (`src/door.ts` -> `src/door.ts.script` and `src/door.lua`) holding those exports and the whole closure they reach, including an `export { … }` list, a sibling of the same `const a = 1, b = 2;` declaration, and the companion's own `require` of an import that closure needs. The component requires the companion rather than redeclaring it, so an exported object is one table for the script and every importer; companions are not supported under a configured `outDir` ([script state](./script-state.md#exporting-a-value-from-a-script-itself)).
