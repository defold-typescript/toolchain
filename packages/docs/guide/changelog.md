...
- **Editor scripts get a typed editor API.** The new `@defold-typescript/types/editor-script` entrypoint exports the `defineEditorScript` and `defineEditorCommand` factories and carries the editor VM without the game runtime — `editor.get`/`bob`/`execute`/`transact`/`tx.*` plus the VM's own `http`, `json`, `zip`, `zlib`, `pprint` and `tilemap.tiles` libraries — while a command wrapped in `defineEditorCommand`, or registered through `editor.command`, types its `active`/`run` opts bag from its own `query`, so a `"one"` selection reads as a single node, a `"many"` one as a list, a member the query never declared is a compile error, and both factories erase to plain command tables the editor can load. `editor.ui` and `editor.prefs` are still untyped, and [`wall`](./wall.md) does not target [editor-script](./editor-scripts.md) directories yet — set `types` by hand.
