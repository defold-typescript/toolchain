...
- **Forked library documentation** — a forked library's `/api` page now shows upstream's own prose for the members its fork documents nowhere: 257 descriptions across 14 libraries, each marked with a `U` dot so imported text is never presented as first-party. The fork's own wording always wins where it has any, `@param`/`@return` lines never come along, and a brief never just restates the name of the symbol it sits under.
