...
- **[`build`](./build.md) and [`watch`](./watch.md) now fail on three output arrangements they used to emit silently, each naming the file and what to change.** Type-only imports, and a `require` with no TypeScript source behind it such as a library's Lua or `lldebugger.debug`, are still left alone.
