...
- **The scaffold's managed `.gitignore`, `biome.json` and `Lua.workspace.ignoreDir` rules now follow your `tsconfig.json` `include` instead of assuming `src/`.** [`init`](./init.md) ignores generated component output by its `.ts.` suffix rather than by folder, lints the roots you actually compile, and claims a folder for the Lua language server only when no hand-authored `.lua` sits under it; a re-init (or an [`upgrade`](./upgrade.md)) retires the old `src`-shaped rules from a project that carries them, so a generated `.lua` module now shows up in `git status` unless you set `outDir`, and a `biome.json` carrying comments is reported for you to edit by hand rather than rewritten.
