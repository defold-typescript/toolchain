...
- Library surfaces corrected against their upstream pins, so code written to the old shapes stops type-checking until it is updated.
  - **[defmath](/api/defmath)** (`c67c2273`) — `in_triangle` takes `(x0, y0, arr)` with a six-number array instead of eight loose coordinates; the eight-parameter form upstream also defines is shadowed at runtime and never ran.
  ...
  - **[gooey](/api/gooey)** (`10.5.3`) — `dynamic_list`, `horizontal_dynamic_list` and `vertical_dynamic_list` drop the `root_id` second argument upstream never took, `dynamic_list` and `static_list` drop a trailing `is_horizontal`, and `vertical_scrollbar` gains upstream's `config` between `action` and its callback — so calls written to the old shapes were passing arguments into the wrong slots.
  - **[metrics](/api/metrics.fps)** (`1.2.1`) — `create`'s `position` and `color` arguments are a [`Vector3`](/api/Vector3) and a [`Vector4`](/api/Vector4) instead of `string`; a string there never worked at runtime, so pass `vmath.vector3`/`vmath.vector4` values, or the module's own `POSITION`/`COLOR` defaults.
  ...
  - **[nakama](/api/nakama)** (`v3.4.0`) — every request-taking call declares upstream's positional parameters instead of a body object, so `authenticate_custom(client, id, vars, create, username)` replaces the collapsed table form; all 84 retry-capable calls accept a trailing `retry_policy` and `cancellation_token`; and the socket surface (`socket_connect`, `socket_send`, the ten `on_*` listeners and the `create_*_message` constructors) is gone from this module, which never exported it — a socket binding needs `nakama.socket` vendored as its own module, and `create_socket` is the one door this module opens to it.
  - **[nakama.engine.defold](/api/nakama.engine.defold)** (`v3.4.0`) — `http` takes `retry_policy` and `cancellation_token` before its callback, and `socket_send` drops its third argument; both now match what upstream really defines.
  - **[orthographic](/api/orthographic.camera)** (`3.6.3`) — `add_projector`, `get_projection_id` and `use_projector` are gone; upstream's body for each only raises *"is deprecated"*, so any call already failed at runtime. `world_to_screen` drops its third `adjust_mode` argument, which upstream silently ignored — remove it from your calls, and adjust the result yourself if you were relying on it. The constants `MSG_USE_PROJECTION` and `ORTHOGRAPHIC_RENDER_SCRIPT_USED` are gone too; upstream defines neither, so reading either returned `nil` and posting `MSG_USE_PROJECTION` reached nothing.
  ...
  - **[yagames](/api/yagames)** (`0.19.0`) — `banner_init`, `banner_create`, `banner_delete`, `banner_refresh` and `banner_set` give way to the sticky-banner API upstream documents (`adv_show_banner_adv`, `adv_hide_banner_adv`, `adv_get_banner_adv_status`); the sitelock functions `add_domain`, `verify_domain`, `get_current_domain` and `is_release_build` are gone because the module never exported them; and `payments_get_catalog` takes upstream's `(options, callback)`, so a lone callback no longer type-checks.
...
- Libraries whose upstream ships no usable structured source — or only one too lossy to generate from — are now first-party forked `.d.ts`s maintained in this repo: declaring the upstream dependency resolves and materializes the types through [`resolve`](./resolve.md), and each docs page imports the library under its module id. Type surfaces generally carry over unchanged — only their provenance and end-to-end resolution move; [defsave](/api/defsave), [orthographic](/api/orthographic.camera), [yagames](/api/yagames), [metrics](/api/metrics.fps), [gooey](/api/gooey) and [boom](/api/boom) are the exceptions, where being maintained here is what let a surface be corrected against its pin.
  ...
  - **[gooey](/api/gooey)** (`britzl/gooey` `10.5.3`) — imported as `import * as gooey from "gooey.gooey"`, with its import unchanged; its reference page moves from `/api/gooey.gooey` to `/api/gooey`. The fork is what let the module be completed and corrected against `gooey.lua` — `gooey.group` now takes the `action_id` and `action` arguments the runtime requires, so the four-argument call every upstream example writes finally type-checks.
...
- **Forked library surfaces** — every forked library is now measured against its own upstream Lua on two separately ratcheted axes, functions and constants, or explicitly recorded as unmeasurable, so one nobody has checked can no longer read as clean: 35 of the 36 modules carry a committed parity report and now cover their upstream's whole surface on both axes, and only [starly](/api/starly) says why it could not be measured. A library's ambient globals, such as [deftest](/api/deftest)'s assertions and most of [boom](/api/boom), are counted apart rather than reported as names its module invented; [authoring forked library types](./authoring-forked-library-types.md) documents why a perfect score on one axis says nothing about the other.

- **Forked library documentation** — a forked library's `/api` page now documents constants as well as functions: 61 constants gained the description their fork already carried, eleven members whose upstream comment is written in a shape the LuaDoc reader has to decline were written up in the fork's own words ([zzfx](/api/zzfx)'s four calls, [nakama](/api/nakama)'s three cancellation helpers and four others), and 254 members across 14 libraries fall back to upstream's prose under a `U` dot marking it as borrowed. The fork's own wording always wins where it has any, `@param`/`@return` lines never come along, and a brief never just restates the name of the symbol it sits under.

- **[panthera](/api/panthera)** (`Insality/panthera`, tag `runtime.8`) — the Panthera animation runtime joins the corpus as a new library: declare the dependency and [`resolve`](./resolve.md) materializes a typed `panthera.panthera` module covering `create_go`/`create_gui`/`create`, `play`/`play_tweener` and the animation-state shape. `defold-tweener` is a runtime companion only — panthera's types resolve without it.
...
- Forked libraries declare members their upstream really defines that the previous bindings left out.
  - **[richtext.color](/api/richtext.color)** — color parsing: `parse`, `parse_hex` and `parse_decimal`.
  - **[nakama.util.log](/api/nakama.util.log)** — the two logging-mode setters, `custom` and `format`.
  - **[persist](/api/persist)** — `exists`, which reports whether a saved file is there.
  - **[bzAnim](/api/bzAnim)** — `setMaxPoints`, `registerController` and `unregisterController`.
  - **[defcon](/api/defcon)** — `register_module` takes upstream's optional second `name` argument, and the module exposes `print`, `pprint` (the engine's originals, captured so `stop()` can restore them) and `server`.
  - **[orthographic](/api/orthographic.camera)** — `get_automatic_zoom` and `set_automatic_zoom`, for reading and toggling a camera's automatic zoom level, and the `MSG_SET_AUTOMATIC_ZOOM` message that does the same by post.
  - **[monarch](/api/monarch.monarch)** — the seven live message hashes `TRANSITION_DONE`, `TRANSITION_SHOW_IN`, `TRANSITION_SHOW_OUT`, `TRANSITION_BACK_IN`, `TRANSITION_BACK_OUT`, `FOCUS_GAINED` and `FOCUS_LOST`, plus `register`, upstream's alias of `register_proxy`. The grouped `TRANSITION` and `FOCUS` objects the previous binding offered instead are now marked deprecated, as upstream marks them.
  - **[platypus](/api/platypus)** — `SEPARATION_RAYS` and `SEPARATION_SHAPES`, the two values for `collisions.separation`. Upstream warns at runtime that only raycast separation is still supported.
  ...
  - **[yagames](/api/yagames)** — 22 members covering whole feature areas: the events API (`event_on`, `event_off`, `event_dispatch`), the multiplayer-sessions trio, the five `features_*` calls, the `player_*` getters (`player_get_paying_status`, `player_get_personal_info`, `player_get_signature`, `player_get_mode`, `player_is_authorized`), the shortcut-prompt pair, `flags_get`, `is_available_method`, `server_time` and `device_info_is_tv`.
  - **[nakama](/api/nakama)** — the subscriptions area (`list_subscriptions`, `get_subscription`, the `validate_subscription_*` calls and their constructors), the cancellation pair `cancellation_token` and `cancel`, plus `delete_account` and `delete_tournament_record` — 16 members upstream defines that the previous binding never declared. Its 12 enum constants are declared too, typed as their literal values so `op === nakama.APIOPERATOR_BEST` narrows: the five `APIOPERATOR_*`, three `APISTOREENVIRONMENT_*` and four `APISTOREPROVIDER_*` tokens.
  - **[metrics](/api/metrics.fps)** — both modules expose the drawing defaults `POSITION`, `COLOR` and `FORMAT`, which upstream uses for any `create` argument you omit.
  - **[dicebag](/api/dicebag)** — `bags` and `tables`, the keyed state every `bag_create`/`table_create` writes into and every draw reads back.
  - **[rendy](/api/rendy)** — `cameras` plus `display_width`, `display_height`, `window_width` and `window_height`. Upstream exposes these for its own render script and advises against manipulating them directly, which each one's doc-comment now says.
  - **[monarch.transitions.gui](/api/monarch.transitions.gui)** — the twelve transitions (`slide_in_*`, `slide_out_*`, `scale_in`/`scale_out`, `fade_in`/`fade_out`) are declared as functions instead of constants of an opaque callable type, so the reference page documents each one's arguments rather than showing a bare variable.
