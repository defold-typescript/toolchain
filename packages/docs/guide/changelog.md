...
- **[`build`](./build.md) and [`watch`](./watch.md) now warn about address literals that cannot resolve.** Both checks name the file without failing the build — on `build`, on every `watch` rebuild, and on every save that moves the address universe — and `--json` repeats each finding as a structured entry, on the `sceneTypes` event too, so an agent never parses the prose.
  ...
  - **A world your script cannot reach.** A socket-qualified address like `mylevel:/enemy` written at a slot that resolves in the caller's own world — every handle-resolving [`go`](/api/go) call — is reported with the world your script actually runs in, since only `msg.post` and `msg.url` cross a collection proxy. A bare or relative address, and any script whose world the scene walk could not resolve, is never reported; the entries are `crossWorldAddresses`.
