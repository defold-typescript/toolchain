...
- **The watch loop now works with the running editor.** [`watch --hot-reload`](./watch.md#hot-reload) — scaffolded as the `defold-typescript:watch-hr` mise task — pushes every successful rebuild into the running game, reloading nothing when a build fails and reloading the editor's extensions instead when the emit was an [editor script](./editor-scripts.md); with or without that flag, `watch` also prints the running game's [runtime errors and their tracebacks](./watch.md#runtime-errors-in-the-terminal) into your terminal, and [`build`](./build.md) names the editor it found. A reload does not re-run `init`, so whatever it set up belongs in [`on_reload`](./script-lifecycle.md#hot-reload-and-on_reload), and Defold's own build errors still reach only the editor's Build Errors tab.
