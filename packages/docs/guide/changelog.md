...
- The guide no longer suggests that a sufficiently pure npm package can work: [TypeScript vs Lua](./typescript-vs-lua.md)'s "self-contained Lua" claim is corrected, and [TypeScript gotchas](./typescript-gotchas.md) gains an entry showing the exact build failure, why `node_modules` is never read, and how to vendor the source under `src/` instead. That entry now shows the output paths a scaffolded project actually produces — `src/vendor/pure-pkg.lua` beside `src/main.ts.script`, because the scaffold configures no `outDir` — and says that include-base stripping applies only when one is configured.
