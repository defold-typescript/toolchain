...
- The `proto` library types (`Insality/defold-proto`, pinned tag `1`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as druid and log, and carry the maintained-here pin in the docs. The surface follows upstream's annotations, so it differs from the previous hand-written binding: `get`/`decode`/`verify` now return the LuaLS-native `LuaTable` rather than a `{ [key: string]: unknown }` map, `set_logger` takes a typed `proto_logger`, and the full encoding API is now surfaced.
- The `saver.saver` and `saver.storage` library types (`Insality/defold-saver`, pinned tag `8`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as proto and log, and carry the maintained-here pin in the docs. The surface follows upstream's annotations, so it differs from the previous hand-written binding: `init`, `save_game_state`, `get_save_path`, and `set_logger` now take their typed optional config/logger arguments, and the full save and key-value storage APIs are surfaced.
- The `immutable` library types (`paweljarosz/lua-immutable`, pinned tag `v1.1`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as proto and saver, and carry the maintained-here pin in the docs. The surface follows upstream's annotations, so it differs from the previous hand-written binding: `make` now returns the typed `Immutable` interface rather than a generic `Readonly<T>`, and `is_immutable` stays a boolean predicate. (LuaLS class heads carrying a same-line description now parse to the bare identifier, so `Immutable` resolves instead of falling to `unknown`.)
- The `squid` library types (`paweljarosz/squid`, pinned tag `1.2`) are now maintained in this repo, regenerated from upstream's LuaLS annotations through the same pipeline as proto and immutable, and carry the maintained-here pin in the docs. The previous hand-written binding typed only `save_logs` and `get_config().is_enabled`; the regenerated surface exports the module log-level constants (`squid.TRACE`…`squid.ERROR`, `squid.ALLOWLIST`), keeps `get_config` — now returning the fully typed `SquidConfig` — adds the module logging API (`init`, `trace`/`debug`/`info`/`warn`/`error`, `log`, `set_config`), and makes `new()` return a typed `SquidInstance` whose instance methods (`log`, `trace`/`debug`/`info`/`warn`/`error`, `set_allowed`, `save_logs`, `init`, `final`) are fully typed.
