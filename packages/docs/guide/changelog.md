...
- **Forked library surfaces** — every forked library is now measured against its own upstream Lua or explicitly recorded as unmeasurable, so one nobody has checked can no longer read as clean: 33 of the 36 modules carry a committed, ratcheted parity report, and the remaining three say why they could not be measured. [nakama](/api/nakama) is still the outlier — 4 of upstream's 156 functions declared with the right parameter list — and [authoring forked library types](./authoring-forked-library-types.md) documents the check, including why a perfect score over a mostly-constant module is a thin result rather than a good one.
