...
- **Forked library surfaces** — every forked library is now measured against its own upstream Lua on two separately ratcheted axes, functions and constants, or explicitly recorded as unmeasurable, so one nobody has checked can no longer read as clean: 33 of the 36 modules carry a committed parity report, and the remaining three say why they could not be measured. [nakama](/api/nakama) is the outlier on both — 4 of upstream's 156 functions declared with the right parameter list, and none of its 12 constants — and [authoring forked library types](./authoring-forked-library-types.md) documents why a perfect score on one axis says nothing about the other.
