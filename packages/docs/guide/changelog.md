...
- **The CLI now works with the running editor.** A reload does not re-run `init`, so whatever it set up belongs in [`on_reload`](./script-lifecycle.md#hot-reload-and-on_reload), and Defold's own build errors still reach only the editor's Build Errors tab.
  ...
  - **Console errors name your TypeScript** — on both surfaces a reported location now carries the authored `.ts` line and column in front of the generated chunk one (`src/main.ts:5:11 (/src/main.ts.script:4)`), read from the source map the build already writes beside every output. It is best-effort: a chunk with no map, a generated line the map does not cover, or a map that does not parse is left exactly as the console sent it, since a wrong `.ts` line would be worse than an honest generated one. Under `reload --json` the raw `consoleErrors` are unchanged and the translations arrive beside them as `consoleErrorLocations`.
