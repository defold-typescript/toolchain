...
- **[`resolve`](./resolve.md) can no longer be made to write outside `.defold-types/dependencies/`.** A dependency archive whose scene entry carried a `..` path could overwrite a file in your own project; such an entry is now refused and named on `stderr`, and the rest of that archive still unpacks.
