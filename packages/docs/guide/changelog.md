...
- **Platform and caution markers in the API reference now show as platform logos and icons whose label appears on hover or keyboard focus.** Notes such as "iOS only" or "attention" on engine and library pages like [`sys`](/api/sys) and [`adinfo`](/api/adinfo) used to vanish or read as `[icon:ios]` bracket text; library cards and symbol tooltips now drop the brackets.
