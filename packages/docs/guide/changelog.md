...
- **Forked library surfaces** — every forked library is now measured against its own upstream Lua on two separately ratcheted axes, functions and constants, or explicitly recorded as unmeasurable, so one nobody has checked can no longer read as clean: 35 of the 36 modules carry a committed parity report, and only [starly](/api/starly) says why it could not be measured. [nakama](/api/nakama) is the outlier on both — 4 of upstream's 156 functions declared with the right parameter list, and none of its 12 constants — while a library's ambient globals, such as [deftest](/api/deftest)'s assertions and most of [boom](/api/boom), are counted apart rather than reported as names its module invented; [authoring forked library types](./authoring-forked-library-types.md) documents why a perfect score on one axis says nothing about the other.

- **Forked library documentation** — a forked library's `/api` page now shows upstream's own prose for the members its fork documents nowhere: 244 descriptions across 14 libraries, each marked with a `U` dot so imported text is never presented as first-party. The fork's own wording always wins where it has any, `@param`/`@return` lines never come along, and a brief never just restates the name of the symbol it sits under.
