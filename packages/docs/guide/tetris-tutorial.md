...
├─ main/main.collection  # boot scene init writes
├─ src/main.ts           # sample script — you replace it in Step 05
├─ tsconfig.json
├─ package.json
├─ mise.toml
├─ biome.json
├─ .gitignore
└─ .vscode/              # editor settings, snippets, debug launcher
```
...
1. In the **Assets** pane, right-click the project root → **New… → Folder**, name it `input` (if it does not exist). Right-click `input` → **New… → Input Binding File**, name it **game.input_binding**.
2. Open it and add five **Key Triggers**:
...
3. Open **game.project → Input → Game Binding** and point it at `/input/game.input_binding`.
...
**State.** `init` returns a **`BoardSelf`** — the generated GUI nodes, the grid model, the active piece (index, rotation, pivot `px`/`py`), the gravity timer, and the score/level fields. A bare `self` is `noImplicitAny`-illegal, so the interface is the type you add. It first posts `acquire_input_focus` to `.`; without it, no key reaches `on_input`. The five input names are hashed once at module scope (`hash("left")` through `hash("hard_drop")`).
...
**Render.** `redraw` is "model → colors": each frame it composes the locked grid plus the falling piece on top, then recolors every cell node. The model only ever holds locked blocks, so movement never erases anything — the next frame composes fresh.
