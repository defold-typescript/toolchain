...
Tetris is the perfect second Defold project. It has exactly one moving thing, a fixed-step clock, a tiny rules engine, and no physics — so every concept you learn here is the concept, not a workaround. We build the whole board from code-generated GUI nodes (no sprites placed in the editor), drive it on a fixed timestep, and lean on the type system to catch the mistakes that silently break a Lua build.
...
This is the part tutorials usually skip — and here it's tiny, because the board draws itself from code. Defold is editor-first (scripts attach to components, components to game objects, game objects to collections), yet the playfield has **no hand-placed nodes**: the script generates every cell at runtime (Step 5). You wire up an empty GUI scene and a game object, and that's it — no sprite, no atlas, no factory.

### 2a — Create the board GUI scene

The board is a **GUI scene** whose nodes are all created from code, so the scene file itself is nearly empty.
...
3. Raise **Max Nodes** to at least `600`: the board creates `COLS × ROWS × 2` = 400 box nodes (a fill plus a border each), over the default budget of 512.
...
1. Open **main.collection**. Right-click root → **Add Game Object File** → choose **board.go**. Set its **Id** to `board`. Its position doesn't matter — GUI nodes live in screen space, so the script centers the grid itself (Step 5).
...
That's the entire scene: two game objects, each hosting a GUI scene — one for the board, one for the HUD. Not a single cell is placed by hand; the script draws all 200.
...
Here's the central trick: we don't place a single node in the editor. At startup the script generates the whole `COLS × ROWS` grid with `gui.new_box_node`, then each frame it only changes each node's **color** to reflect the model. Each cell is two stacked box nodes — a `border` box with a smaller `fill` box on top — so `GAP` (space between cells) and `BORDER` (frame thickness) are single variables you tune from one place.
...
The whole renderer is "model → colors": each frame, walk the grid and set every cell's fill and border color. Empty cells (`0`) paint a dim well color (not transparent), so the playfield grid is always visible; filled cells get their piece color plus a darker border. Row `0` is the top of the board, so we flip `r` against `ROWS` for the y-position, since GUI's y-axis points up.

> [!NOTE]
> **Why generate, not place** Nodes created from code (`gui.new_box_node`) need no editor work and scale with `COLS`/`ROWS` — change the board size and the grid follows. Recoloring an existing node every frame is far cheaper than creating and deleting nodes as pieces move: build once, recolor forever. The only editor knob is the scene's **Max Nodes**, which must cover `COLS × ROWS × 2` (we set 600 in Step 2).
...
Because all four helpers test against the `grid.ts` model and never touch a GUI node, every movement is "just data" — which is exactly why we kept that module engine-free. The renderer in Step 5 reflects the result _after_ the model changes.
...
Score and level live on their **own** GUI scene, separate from the board's. Add a **.gui** file with two text nodes (`score`, `level`) plus a hidden `gameover` node, then drive it from a `.gui_script`. Each GUI scene gets one script file with a single `defineGuiScript` export — the board and the HUD are two such pairs.
...
And the board's side of the conversation — one helper that posts the current numbers, but only once the HUD has **registered** (a gui script can't call `go.exists`, so the HUD announces itself; the board records that in `on_message`). Because `msg.post` is asynchronous (delivered next frame), the HUD always shows last frame's values, which for a score display is imperceptible:
...
> [!NOTE]
> **Editor step** Create the HUD object: in `main.collection`, add a **GUI** component to a new game object with **Id** `hud`, point it at your `.gui` file, and attach `hud.ts.gui_script`. Its message url becomes `/hud#hud` — exactly what the board posts to. Because a gui script can't call `go.exists`, the HUD **registers** with the board in its own `init` (`msg.post("/board#board", "hud_ready")`) and the board posts only after that arrives — so the game runs cleanly with or without the HUD, and `msg.post` never targets a missing instance. The runnable example ships a ready-made `main/hud.gui` (the builtin `/builtins/fonts/default.font`, three text nodes `score`/`level`/`gameover`) already wired into `main.collection` — copy it as a starting point.

## 10 — Run it, then ship it

With `watch` running, hit `Project → Build` (or `Cmd/Ctrl`+`B`) in the Defold editor. The engine loads `main.collection`, your board script generates the grid's 400 GUI nodes (a fill plus a border per cell), and gravity starts ticking.

1. If the board is invisible, check `board.gui`'s **Script** is `/src/board.ts.gui_script` and its **Max Nodes** is at least `600` (too low and `gui.new_box_node` runs out partway).
...
These are the buildable files from `docs/examples/tetris-tutorial/src/`. The complete `board.ts` types every `self` through a named **`BoardSelf`** interface — the teaching excerpts above wrote a bare `self` for brevity, but a bare parameter is `noImplicitAny`-illegal. The HUD's `hud.ts` imports **`defineGuiScript` from `@defold-typescript/types`** (the main entry). Nothing else differs from the walkthrough.
...
You have a playable game. The natural next exercise is one feature every Tetris player expects: **a preview of the piece that's coming next**, drawn above the playfield so you can plan ahead.

The hard part is not the rendering — it is the **peek**. `nextPieceIndex()` in `src/pieces.ts` *consumes* from the 7-bag, so calling it once for "what's next" leaves the bag empty for the actual spawn. Refactor it into a peek/advance pair: a function that reads without consuming (`peekNextPieceIndex()`), and the existing one becomes "pop, then refill". The board's `onLocked` calls `peekNextPieceIndex()` *before* spawning, so the next-next piece is already in hand.

Two reasonable places to draw the preview:

- **In the HUD.** Reuse the `hud.ts` message protocol: extend `set_hud` with a `next: number` field (the color index 1..7), and have `hud.ts` paint a small 4-cell cluster out of the same `TINTS` palette, anchored above the playfield. This keeps the preview logic out of `board.ts` and reads naturally — the HUD already owns "out-of-board text and tiny graphics."
- **In `board.ts` itself, above the grid.** Generate a second 4x4 set of `gui.new_box_node`s above `ORIGIN_Y`, repaint them the same way `redraw` repaints the grid. Cheaper to wire (no protocol change), but the board script now owns two render surfaces.

Either way, the change is small in `board.ts` and the new node tree; the lesson is the **peek-vs-consume split** in `pieces.ts`, which generalizes to "the bag is a stateful randomizer" — the same shape a real Tetris uses for hold-piece and queue-length previews.
