...
## Animation id completions inside the `play_flipbook` calls
...
Only blocks declared as `animations { id: … }` are offered, in both slots. A bare image in an atlas is not, and neither is [`model.play_anim`](/api/model), whose names come from an animation set rather than an atlas.
