...
marked with a no-typed-API icon, and each page describes what the library
ships, how a project adds and uses it, and whether it registers a Lua API that
is simply not typed. `resolve` still reports each one as `asset-only, skipped`.
