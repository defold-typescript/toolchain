...
Defold libraries that ship no `.script_api` are listed there too, as cards that
link to the GitHub repo and say they have no typed API, so you can see they
exist; `resolve` reports each one as `asset-only, skipped`.
