...
- [reload](./reload.md) — push one reload into the running game and read the editor console for a bounded window, so a script learns whether the reloaded code ran instead of trusting the editor's "queued" response.
