...
```ts
function fadeIn(node: Opaque<"node">, easing: gui.Easing, playback: gui.Playback) {
...
**How we pin this in the type tests.** `packages/types/test-d/guide-snippets.ts` asserts the constant is assignable to `number` (backward compatibility) and to its own brand, while a bare `0` assigned to the brand carries `@ts-expect-error`. If the brand is dropped or widened, the expected error disappears and the typecheck gate fails. `packages/types/test-d/constant-union-aliases.ts` does the same for the aliases: representative members are assignable to their alias, and a constant from a sibling group is not. Exhaustive member-set fidelity is held elsewhere: `packages/types/test/constant-union-alias-coverage.test.ts` catches a brand union left without a name, and `packages/types/test/constant-slot-expansion-drift.test.ts` checks each documented slot against the reference documentation.
...
**A table whose *keys* are constants is a `LuaMap`, not an object literal.** A brand is not one of the three types a TypeScript index signature accepts (`string | number | symbol`), so a generated declaration cannot describe "a table keyed by these constants" as an index signature — it uses a `LuaMap` whose key parameter carries the brand, as `render.clear`'s `LuaMap<render.ClearBufferKey, number | Vector4>` does. Writing one is its own subject: see [Lua table constructs](./lua-table-constructs.md).
...
The engine functions follow the same sentinel: a documented `nil` return is typed `T | undefined`, so narrow it explicitly with `!== undefined` before use. The guard **precisely tests absence** and transpiles to `~= nil` in Lua — the not-equal counterpart of the `== nil` that the equal `=== undefined` / `=== null` checks emit — so it does not lean on the divergent [TS/Lua truthiness](#if-x-truthiness-differs--0-and--are-truthy-in-lua) — where `0` and `""` are truthy once transpiled — the way a bare `if (value)` would.
...
**How we pin this in the type tests.** `packages/types/test-d/sys-info-table-fields.ts` asserts `sys.get_sys_info().system_name` is assignable to `string` and that a bogus field carries `@ts-expect-error`; `packages/types/test-d/example-shaped-table-curations.ts` pins the example-proven `liveupdate.get_mounts()` and `tilemap.get_tile_info()` shapes; `packages/types/test-d/physics-set-shape-fields.ts` pins the variant `physics.get_shape()` record — `type` reads as `number`, while `diameter` and `height` need narrowing before they are. The fidelity audit and the emitter share the table recovery and curation data (`packages/types/src/emit-dts.ts`), so the `recordTables` loss gate and the emitted surface cannot drift; if a recovered field's type regressed to `unknown` the audit's `unknownTokens` would surface it.
...
**Symptom.** A `msg.post` that reads naturally in TypeScript silently fails to reach its target — the receiver's `on_message` never fires, and nothing compiles wrong. The same applies to every API that takes a URL string (`go.get`/`go.set`, `msg.url`, `factory.create`, …). A literal socket copied from a Lua manual example, like `"main:/manager#controller"`, is the usual culprit: it resolves only when the target collection is actually *named* `main`.

```ts
msg.post("main:/enemy#ai", "alert");            // resolves only if the target collection's Name is "main" — fragile
msg.post("/enemy#ai", "alert");                 // robust: absolute path inside the current world
msg.post("#health", "damage", { amount: 10 }); // sibling component on the same game object
```

**Why.** A Defold URL is `[socket:][path][#fragment]` — three parts, the first optional. The `path` is the target object's id, the `#fragment` is the component on it, and an address that **starts with `/` is resolved from the root of the current game world** (the bootstrap collection loaded at game start). The `socket:` segment names *which game world* the target lives in, and it earns its keep only with collection proxies: a world's socket name is the **`Name` property of its collection file** — "The new world gets its name from the *Name* property in the collection file." The bootstrap collection is named the same way (it too is given a unique name from *its* `Name` property), so there is no special socket you address it through — inside that one world you never write the `socket:` segment at all. The trap is that a collection's `Name` is a property distinct from its folder: the editor's `main/main.collection` is a *file path*, while the `Name` you address the world by is a separate field (commonly the project default `default`, not `main`). So a literal `"main:…"` borrowed from a manual example breaks the moment your bootstrap collection is named anything other than `main`. The two-arg form `msg.url(socket, path)` is a separate runtime trap — only the no-arg, one-string, and three-arg forms are runtime-supported, so `msg.url("main", "camera")` is a runtime error. All three positions of the three-arg form are required, but `socket` may be `undefined` (Lua `nil`) to address the current game world. The tightened `msg.url` overloads in `packages/types/src/msg-overloads.d.ts` reject that two-arg call at compile time, so the typings are the warning.

**Typed alternative.** For anything in the **same world** (everything in your bootstrap collection) address it without a socket: `"camera"` (a sibling instance by id, relative to the current component), `"/camera"` (the same instance by id, absolute from the world root), or `"#health"` (a sibling component on the same game object). Reserve `socket:` for reaching into a **collection-proxy-loaded** world, and pass the *target* collection's `Name` — the collection-proxy manual's own cross-world example is `msg.post("mylevel:/myobject", "hello")`, where `mylevel` is the loaded collection's `Name`, not the proxy component's id. For `msg.url`, use one of the three typed overloads — `msg.url()` (the current message's url), `msg.url("main:/manager#controller")` (one string), or the three-position `msg.url(hash("level1"), hash("/door"), hash("script"))`, whose `socket` may be `undefined` for the current world — never the two-arg form.

**How we pin this in the type tests.** `packages/types/test-d/msg-url-arity.ts` asserts the tightened `msg.url` overloads — `msg.url()` and `msg.url("main:/manager#controller")` type-check, while the two-arg `msg.url("main", "camera")` carries `@ts-expect-error` — so the runtime-invalid arity is rejected at compile time; widen the overloads to accept it and the expected error vanishes, failing the typecheck gate.
...
The scaffold configures no `outDir`, so generated Lua lands next to its source: that emits `src/vendor/pure-pkg.lua`, beside the `src/main.ts.script` your entry point compiles to. Three things to know:

- The scaffold's `include` is `["src/**/*.ts"]`. Vendored source must sit under an include base or it is never compiled at all.
- The output path strips the **longest matching** include base, but **only when `compilerOptions.outDir` names a directory other than `"."`** — an absent `outDir`, `""`, and `"."` all mean alongside output, where the tree mirrors the source tree exactly and nothing is stripped. With an `outDir` of `"build"` and `["src/**/*.ts", "vendor/**/*.ts"]`, `vendor/pure-pkg.ts` lands flattened at `build/pure-pkg.lua` — in the outDir root, where it can collide with a `src/` name. Vendoring under `src/vendor/` keeps the folder in the output either way.
