...
- The output path strips the **longest matching** include base, but **only when `compilerOptions.outDir` is configured** — with no `outDir` the output tree mirrors the source tree exactly. In `outDir` mode (say `"build"`) with `["src/**/*.ts", "vendor/**/*.ts"]`, `vendor/pure-pkg.ts` lands flattened at `build/pure-pkg.lua` — in the outDir root, where it can collide with a `src/` name. Vendoring under `src/vendor/` keeps the folder in the output either way.
