...
The engine functions follow the same sentinel: a documented `nil` return is typed `T | undefined`, so narrow it explicitly with `!== undefined` before use. The guard **precisely tests absence** and transpiles to the same `== nil` in Lua, so it does not lean on the divergent [TS/Lua truthiness](#if-x-truthiness-differs--0-and--are-truthy-in-lua) — where `0` and `""` are truthy once transpiled — the way a bare `if (value)` would.
