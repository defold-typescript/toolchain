...
**How we pin this.** `packages/types/test/guide-docs.test.ts` asserts this section's heading, its front-digest link, and that every `./typescript-gotchas.md#…` link on [TypeScript vs Lua](./typescript-vs-lua.md) resolves to a real heading here — so renaming either side alone fails the gate. It also pins this section's output paths against the scaffold's no-`outDir` layout and requires the include-base stripping bullet to name the `""` and `"."` alongside exceptions, so the wording cannot drift back to a claim that any configured `outDir` strips.
