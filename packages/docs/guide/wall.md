...
the installed entrypoint and are unaffected. That redirect is the only module
mapping a wall manages: mappings you declare in the wall's own `tsconfig.json`
and in the root config keep resolving inside the walled directory, and dropping
the pin removes only what walling added.
