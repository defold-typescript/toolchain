...
configured pattern reaches a writable path inside the project — an `include` of
a single exact file, or one that only points outside it — `init` writes no
starter and leaves the collection with no component, reporting the patterns it
searched.
