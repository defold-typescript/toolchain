...
4. Convert your Lua scripts to TypeScript — move each Defold lifecycle hook into a `defineScript({...})` method; the [platformer example](https://github.com/defold-typescript/toolchain/tree/main/docs/examples/platformer) is a worked conversion. Or hand the work to a coding agent: step 2 already wrote the `AGENTS.md` / `CLAUDE.md` contract that points it at the toolchain's machine-readable Defold reference, so it converts against the documented API instead of guessing at it. If those files are missing — or `AGENTS.md` has lost its managed block, or `CLAUDE.md` no longer re-exports it with the single line `@AGENTS.md` — run [`init-agents`](./init-agents.md) to restore them.
