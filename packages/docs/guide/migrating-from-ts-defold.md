...
| `@ts-defold/create` — `npm init @ts-defold my-game` (resolves GitHub `tsd-template-*` templates, or a local/remote zip) | `bunx @defold-typescript/cli@latest init path/to/project` — built-in templates via `init path/to/project --template <name>` (`default` / `minimal`) |
